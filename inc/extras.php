 ...
 */
function bpbw_categorized_blog() {
	$category_count = get_transient( 'bpbw_categories' );
	...
	if ( false === $category_count ) {
		...
		set_transient( 'bpbw_categories', $category_count );
	}
 ...
 */
function bpbw_get_attachment_id_from_url( $attachment_url = '' ) {
	global $wpdb;
 ...
 */
function bpbw_copyright_year( $atts ) {
	// Setup defaults.
	$args = shortcode_atts(
		[
			'starting_year' => '',
...
add_shortcode( 'bpbw_copyright_year', 'bpbw_copyright_year', 15 );
 ...
 */
function bpbw_get_custom_logo_url() {
